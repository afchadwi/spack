...
class Xsdk(BundlePackage):
    ...
    depends_on('mfem@4.0.0-xsdk+mpi+hypre+superlu-dist+petsc~sundials+examples+miniapps', when='@0.5.0')
    ...
    depends_on('trilinos@12.18+hypre+superlu-dist+metis+hdf5~mumps+boost~suite-sparse+tpetra+nox+ifpack2+zoltan2+amesos2~exodus+dtk+intrepid2+shards',
               when='@0.5.0 +trilinos')
